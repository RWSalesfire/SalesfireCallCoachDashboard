...
function VerbatimStageCard({ stage, stageNum }: { stage: VerbatimStage; stageNum: number }) {
  return (
...
function GuidedStageCard({ stage, stageNum }: { stage: GuidedStage; stageNum: number }) {
  return (
    <div className="space-y-3">
      {/* Stage header */}
      <div className="flex items-center gap-2">
      ...
      <div className="space-y-8">
        {variation.stages.map((stage, i) =>
          stage.format === 'verbatim' ? (
            <VerbatimStageCard key={stage.title} stage={stage} stageNum={i + 1} />
          ) : (
            <GuidedStageCard key={stage.title} stage={stage} stageNum={i + 1} />
          ),
        )}
      </div>
